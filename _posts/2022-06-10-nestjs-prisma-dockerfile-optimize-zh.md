...
```docker
# CMD ["node", "dist/apps/frontend/main.js"] 改成
ENTRYPOINT [ "npm" ,"run"]
...
//如图[Optimizing builds with cache management (docker.com)](https://docs.docker.com/build/cache/)

![Screenshot 2023-02-25 at 17.38.55](/Users/tuo/Library/Application Support/typora-user-images/Screenshot 2023-02-25 at 17.38.55.png)
...
可以想象每次构建都会重复这个流程，解析package.json和yarn.lock, 然后去远程下载包和元数据到缓存目录（在镜像层里），然后安装到node_modules目录下。每次构建都是一次性的，如果有改动，比如新加了一个包，那么这个流程需要重新再来一次，可以想象这个速度肯定是很慢的，这个缓存甚至有点多余，简直是个累赘。还产生了多余中间产物，就是/usr/local/share/.cache/yarn/v6目录下。
...
所以这个额外的304检查开销应该就是导致缓慢的原因，特别是你的包越多，那么请求的次数和总的时间就多。但是因为用来yarn.lock来精确锁定版本号，是不是可以跳过这个304请求了？ Yarn提供了--prefer-offline的参数来设置缓存策略为离线优先，直接匹配缓存中的数据，如果有直接使用，而不用发额外的过期检查请求。这样只有当包在缓存中没有时，才会发生网络请求，这样理论上应该会快很多。尝试安装另外一个版本*yarn add underscore@1.13.2*
...
{% mermaid %}
flowchart TD
    A[node_modules] --> B{yarn install, prisma generate, nest build}
